...
CREATE TABLE IF NOT EXISTS orders (
  ...
  user_id uuid REFERENCES users(device_id) ON DELETE CASCADE,
...
CREATE POLICY "Users can access their own data"
  ON users
  FOR ALL
  USING (device_id = current_setting('request.jwt.claims')::json->>'sub'::text::uuid);

-- Create policies for orders table
CREATE POLICY "Users can access their own orders"
  ON orders
  FOR ALL
  USING (user_id = current_setting('request.jwt.claims')::json->>'sub'::text::uuid);
